...
# from src.ml_flow.inf_eval import (log_inference_lineage, log_inference_params, log_inference_results, log_inference_start, log_inference_status)
from src.ml_flow.inf_eval import (
    log_inference_start,
    log_inference_params,
    log_inference_results,
    log_inference_lineage,
...
def main(cfg: DictConfig):
    ...
    log_inference_start(cfg, config_name, config_key, experiment_name)
    ...
    if cfg.pipeline.stages.inference:
        try:
            ...
            # })
            # ── MLflow: params ────────────────────────────────────────────────────
            log_inference_params(cfg, latest_ckpt, version_dir, checkpoint_dir)
            ...
            if cfg.model in ["deepseek", "all"]:
                ...
                if prompt:
                    ...
                    log_inference_results(cfg.inference.DEEPSEEK_CONFIG.output_file, "inference.deepseek")
            ...
            log_inference_lineage(cfg, latest_ckpt, version_dir, checkpoint_dir)
            log_inference_status("SUCCESS")   
        
        except Exception as e:  # ← ADDED
            log_inference_status("FAILED")
            print(f"\n Inference failed: {e}")
            import traceback
            traceback.print_exc()
            sys.exit(1)
